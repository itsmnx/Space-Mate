...
// ===== GUI Methods =====
// Add these **after all CLI methods** in this file

std::vector<std::vector<FileInfo>> FileAnalyzer::getDuplicateGroups(const std::string& path) {
    return findDuplicates(path);
}

std::vector<FileInfo> FileAnalyzer::getTempFiles(const std::string& path) {
    return findTempFiles(path);
}

std::vector<FileInfo> FileAnalyzer::getOldFiles(const std::string& path, int days) {
    return findOldFiles(path, days);
}

unsigned long long FileAnalyzer::getPotentialSavings(const std::string& path) {
    unsigned long long duplicateWaste = 0;
    unsigned long long tempSize = 0;

    auto duplicates = findDuplicates(path);
    for (const auto& group : duplicates) {
        for (size_t i = 1; i < group.size(); i++) duplicateWaste += group[i].size;
    }

    auto tempFiles = findTempFiles(path);
    for (const auto& file : tempFiles) tempSize += file.size;

    return duplicateWaste + tempSize;
}
